...
PIPELINE_STAGES = [
    ...
    {"rail_name": "sensitive data output", "label": "Sensitive Data Output Guardrail", "kind": "rail"},
    {"rail_name": "self check output", "label": "Output Policy Guardrail", "kind": "rail"},
...
def _friendly_reason(rail_name: str, blocked: bool) -> str:
    ...
    return {
        ...
        "sensitive data output": "The generated response matched the deterministic sensitive-data filter, so NeMo withheld it before delivery.",
        "self check output": "The generated response did not satisfy the LLM-based output policy, so NeMo stopped it before delivery to the user.",
